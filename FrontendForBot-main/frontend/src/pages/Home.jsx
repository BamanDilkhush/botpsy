...
const FeatureCard = ({ icon, title, children, accent = 'bg-white' }) => (
  <motion.div
    variants={itemVariants}import React from 'react';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { BrainCircuit, BarChart, MessageSquare, Sparkles } from 'lucide-react';
import AnimatedButton from '../components/common/AnimatedButton';
...
const Home = () => {
  return (
    ...
    >
      ...
      >
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <FeatureCard icon={<Sparkles className="w-6 h-6 text-[#2563EB]" />} title="Early Detection">
              Identify developmental patterns sooner with AI-powered insights — helping you take the right steps early.
            </FeatureCard>

            <FeatureCard icon={<BrainCircuit className="w-6 h-6 text-[#10B981]" />} title="Personalized Support">
              Get recommendations tailored to your progress, not generic advice — making guidance more relevant and useful.
            </FeatureCard>

            <FeatureCard icon={<MessageSquare className="w-6 h-6 text-[#F97316]" />} title="Clarity, Not Confusion">
      ...
      {/* CTA */}
      <motion.section variants={itemVariants} className="py-16 px-6 flex justify-center">
        <div className="max-w-3xl w-full text-center rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-xl p-10">
          ...
          <p className="mb-8 text-lg opacity-90">Take the first step towards clarity and support today.</p>
          <Link to="/assessment">
            <AnimatedButton variant="secondary" className="!text-lg !px-8 !py-4 !bg-white !text-indigo-600">
